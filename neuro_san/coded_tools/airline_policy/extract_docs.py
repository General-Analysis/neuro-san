...
class ExtractDocs(CodedTool):
    ...
    def __init__(self):
        docs_root = Path(__file__).resolve().parent / "knowdocs"
        self.default_path = str(docs_root / "Help Center.txt")

        self.docs_path = {
            "Bag Issues": str(docs_root / "baggage" / "bag-issues"),
            "Carry On Baggage": str(docs_root / "baggage" / "carryon"),
            "Checked Baggage": str(docs_root / "baggage" / "checked"),
            "Special Items": str(docs_root / "baggage" / "special-items"),
            "Military Personnel": str(docs_root / "flight" / "military-personnel"),
            "Mileage Plus": str(docs_root / "flight" / "mileage-plus"),
            "Basic Economy Restrictions": str(docs_root / "flight" / "basic-econ"),
            "International Checked Baggage": str(docs_root / "international"),
            "Embargoes": str(docs_root / "international"),
        }
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        if os.path.isfile(directory):
            content = self.extract_txt_content(directory)
            docs[os.path.basename(directory)] = content
            return {"files": docs}
